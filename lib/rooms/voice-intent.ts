...
export const VOICE_MIC_HINT_AR =
  'قل طلبك («أبغا اللائحة…») — يظهر مباشرة في المربع، ثم راجع واضغط Enter (لا إرسال تلقائي)'

export const VOICE_HOW_TO_AR = `كيف يشتغل الصوت والوكلاء:
• الميكروفون: الكلام يُكتب مباشرة في مربع النص أثناء الحديث (مسودة حية).
• أوقف التسجيل → يُنقّح النص عربياً في نفس المربع. عدّله إن لزم.
• أرسل يدوياً بـ Enter (أو زر إرسال) — لا يُرسل الصوت تلقائياً أبداً.
...
• أي رسالة توقظ وكيل١؛ إن كان مشغولاً يُوقظ التالي. اضغط المقعد لإيقاظ/إيقاف يدوي.
حدود صادقة: المسودة الحية قد تكون تقريبية؛ النسخ النهائي يعتمد على الميكروفون والشبكة — راجع دائماً قبل Enter.`
