  ...
  if (DRIVE_OPEN_TOOLS.has(toolName)) {
    throw new Error(
      [
        ...
        'إن تعذّر قراءة بايتات المرفق أعد إرساله مع الطلب — ولن نختار ملفاً آخر بالاسم.',
      ].join(' ')
    )
  }
}
...
export function assertResolvedFileAllowed(
  ...
  throw new Error(
    [
      ...
      `المرجع المطلوب كان «${ref}» — أعد إرسال المرفق إن فُقدت بايتاته؛ لن نختار بديلاً بالاسم.`,
    ].join(' ')
  )
}
...
export function telegramAttachmentMissingAr(name?: string): string {
  return [
    ...
    name ? `«${name}»` : '',
    '— أعد إرسال الملف مع طلبك في التعليق.',
    'لن أستخدم أي ملف من Drive أو الغرفة كبديل بالتشابه في الاسم.',
  ]
    .filter(Boolean)
    .join(' ')
}
