...
export function telegramLargeFileWorkingPathAr(opts: {
  ...
  const sizeMb =
    opts.sizeBytes != null
      ...
      : ''
  return [
    `سجّلت ${name}${sizeMb} — أتابع تلقائياً عبر مسار التنزيل الموسّع (Bot API محلي / جسر الماك) ثم خزنة الغرفة أو Drive.`,
    'عند توفر البايتات أكمل المهمة وأرسل الناتج هنا (sendDocument). لا حاجة لتدخل يدوي.',
  ].join('\n')
}
