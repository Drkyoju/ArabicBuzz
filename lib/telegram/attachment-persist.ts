...
export function telegramFileNeverStoredAr(expectedFilename?: string): string {
  ...
  return [
    ...
    'المهمة معلّقة صامتة — سأعيد البحث تلقائياً في كل المخازن وأكمل بلا طلب إعادة إرسال.',
...
export function telegramLargeFileWorkingPathAr(opts: {
  ...
  const sizeMb =
    opts.sizeBytes != null
      ...
      : ''
  return [
    `سجّلت ${name}${sizeMb} — المهمة في طابور انتظار صامت (مرة واحدة فقط، بلا إلغاء).`,
    'أتابع تلقائياً: Local Bot API → جسر الماك → MTProto عند الحاجة، ثم خزنة الغرفة أو Drive بنفس الاسم.',
    'إن كان hop الماك متوقفاً (نوم الجهاز / نفق مقطوع) تبقى المهمة معلّقة حتى يعود الجسر أو يظهر الملف في الغرفة/Drive.',
    'عند توفر البايتات أكمل وأرسل الناتج هنا. لا حاجة لإعادة إرسال الملف.',
  ].join('\n')
}
