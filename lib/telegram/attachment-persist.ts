...
export function telegramFileNeverStoredAr(expectedFilename?: string): string {
  const name = expectedFilename?.trim()
    ...
    : 'الملف'
  return [
    `${name} ما وصل للتخزين وقت الاستلام (حد تنزيل بوت تيليجرام أو انقطاع).`,
    'للإكمال: ارفعه مرة واحدة إلى ملفات غرفة الفريق على الموقع أو إلى عقل الشركة (Drive) بنفس الاسم تماماً — سأكمل المهمة تلقائياً عند ظهوره.',
    'لن أطلب إعادة الإرسال عبر تيليجرام مراراً، ولن أستبدل ملفك بملف آخر بالتشابه في الاسم.',
  ].join('\n')
}
...
export function telegramLargeFileWorkingPathAr(opts: {
  ...
  const sizeMb =
    opts.sizeBytes != null
      ...
      : ''
  return [
    `سجّلت ${name}${sizeMb} — تنزيل بوت تيليجرام السحابي محدود (~20 م.ب).`,
    'سأكمل عبر خزنة الغرفة / عقل الشركة عند توفر البايتات، ثم أرسل الناتج هنا (sendDocument أو مضغوط إن لزم).',
    'لا حاجة لإعادة إرسال الملف عبر تيليجرام إن رفعته للموقع أو Drive بنفس الاسم.',
  ].join('\n')
}
