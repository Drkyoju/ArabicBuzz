...
export function buildTelegramHelpAr(opts?: {
  ...
  const associationBlock = opts?.inAssociationGroup
    ? [
        ...
        '• «احجز موعد …» → مواعيد الجمعية/الفريق (تقويم الغرفة — ليس تقويمك الشخصي)',
...
export function buildTelegramHelpDomainAr(domain: HelpMenuDomain): string {
  switch (domain) {
    ...
    case 'calendar':
      return [
        '📅 مواعيد الجمعية/الفريق ومهام (توقيت السعودية)',
        '• «احجز موعد غداً ١٠ص» / «موعد» → room_calendar_* (تقويم الغرفة المشترك)',
        '• «كم موعد عندنا؟» / «مواعيد اليوم» → قائمة موجزة (مسار سريع)',
        '• «أضف مهمة …» · «حدّث المهمة» → room_tasks_*',
        '• أجندة الفريق = مواعيد الجمعية/الفريق فقط — ليس تقويمك الشخصي على Google',
        '• تذكير ≈ ساعة قبل الموعد: خاص للمدير فقط أثناء صمت المجموعة (بلا سبام للمجموعة)',
      ].join('\n')
    case 'mail':
      return [
        '✉️ البريد (بطلب صريح فقط)',
        '• «بريد» / «ابحث في البريد عن …» / «وش في البريد؟» → mail_search / mail_read',
        '• «أرسل بريد إلى …» → mail_send (نبرة جمعية مهذبة)',
        '• Gmail الشخصي: gmail_* بعد ربط Google + ربط حساب تيليجرام',
        '• لا يُمسح البريد تلقائياً ولا يُدفع للمجموعة بلا طلب',
