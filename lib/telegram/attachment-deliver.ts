...
export function telegramFileTooLargeAr(opts: {
  ...
  return [
    `بانتظار ${name} — تنزيل بوت تيليجرام السحابي محدود (~${limitMb} م.ب)${
      sizeMb ? ` والملف ≈ ${sizeMb} م.ب` : ''
    }.`,
    'سأكمل عبر Local Bot API / جسر الماك / غرفة الفريق أو Drive عند توفر البايتات بنفس الاسم، ثم أرسل الناتج هنا.',
    'إن أمكن: أرسل ملفاً أصغر، أو ارفعه عبر Drive، أو أعد المحاولة لاحقاً عند عودة الجسر.',
    'لا حاجة لإعادة إرسال الملف مراراً بلا سياق. لن أستبدل ملفك بملف آخر بالتشابه في الاسم.',
  ].join('\n')
}
  ...
  const raw =
    err instanceof Error
      ...
      : typeof err === 'string'
        ? err
        : ''
  return /حد التنزيل|تنزيل بوت تيليجرام|TELEGRAM_MAX_DOWNLOAD|20 م\.ب|بانتظار .*تنزيل بوت|سجّلت .*تنزيل بوت|مسار التنزيل الموسّع|Bot API محلي|Local Bot API|انتظار صامت|طابور انتظار|بانتظار الملف|جسر الماك/i.test(
    raw
  )
}
