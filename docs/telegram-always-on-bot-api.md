...
## حدود النوم على الماك (Mac hop)

الـ hop عبر OrbStack + cloudflared **يعمل فقط والماك مستيقظ ومسجّل دخول**.

| يعمل على تحسين البقاء | لا يزال يوقف الـ hop |
|------------------------|----------------------|
| `npm run mac-hop:install` (watchdog + `caffeinate -dims`) | إغلاق الغطاء على البطارية |
| شاحن الطاقة (AC) متصل | تسجيل الخروج |
| غطاء مفتوح أو clamshell + شاحن + شاشة خارجية | نوم قسري / بطارية منخفضة جداً |
| إعداد النظام: البطارية → خيارات → منع النوم التلقائي والشاشة مطفأة (على المحول) | ترقية OrbStack فوق **1.5.1** |

بعد الاستيقاظ يعيد الـ watchdog الأنفاق بسرعة أكبر (~25ث عند العطل، ~90ث عندما كل شيء سليم). للطوارئ: `npm run mac-hop:watchdog:force`.

**نصيحة:** وصّل الشاحن واترك الجلسة مسجّلة. للمسار الدائم بدون ماك: VPS / `npm run telegram:bot-api:fly`.
